...
adj_ = kregularHarary(8,2);
adj = triu(adj_);
hold on
plot( source.x, source.y, 'ro', 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'g' )
text( source.x, source.y, source.label, 'FontSize', smallfont, 'FontWeight','bold' );

plot( recept.x, recept.y, 'ro', 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'r' )
text( recept.x, recept.y, recept.label, 'FontSize', smallfont, 'FontWeight','bold' );
for i=1:8
line([recept.x(i) recept.x(find(adj(i,:)))], [recept.y(i) recept.y(find(adj(i,:)))],...
   'LineWidth',3,...
   'Color',[.0 .0 .8]);
end
hold off
