function [error_results,opt_dist_result] =post_process_gold3()
global opt_dist
...
function mse_ = imse2(x_,y_)
d_ = abs(x_-y_).^2;
mse_ = sum(d_(:))/numel(d_);
end
...
for j_agent = 1 : opt_dist.nAgents
    for i_consensus=start_step : max_it;
        error_struct.e_cen(i_consensus - start_step +1) = sqrt(imse2(x_cen , x_gt));
        ...
        error_struct.e_gold(i_consensus - start_step +1,j_agent) = sqrt(imse2(x_gold{j_agent} , x_gt));
        
        %% Error Gold vs. centralized
        error_struct.e_gold_vs_cent(i_consensus - start_step +1,j_agent) = sqrt(imse2(x_gold{j_agent} , x_cen));
        
        
        %% Error vs. centralized
        error_struct.e_vs_cen(i_consensus - start_step +1,j_agent) = sqrt(imse2(x ,  x_cen ));
        
        %% Error vs. gold
        error_struct.e_vs_gold(i_consensus - start_step +1,j_agent) = sqrt(imse2(x ,  x_gold{j_agent} ));
        
        %% Error vs. Ground Truth
        error_struct.e_vs_gt(i_consensus - start_step +1,j_agent) = sqrt(imse2(x ,  x_gt ));
        ...
        if  BC_distance(x_cen,P_cen,x,P) - BC_distance(x_cen,P_cen,x_gold{j_agent},P_gold{j_agent})>0.0001 ||...
                (trace( P_gold{j_agent} )/ trace(P))>1 || (trace( P_cen) / trace(P))>1
            disp('Look here')
%             dbstop at 126 in post_process_gold3
        end
    end
...
if (error_mean.e_BC_dist_cent - error_mean.e_BC_dist_gold_vs_cent)>0.0001
    disp('Error: Hybrid performs better than Gold !')
    test_script_p_p_gold_comp
    dbstop at 178 in post_process_gold3
end
