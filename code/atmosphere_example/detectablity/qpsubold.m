...
%   $Revision: 1.5 $  $Date: 2002/03/29 18:31:57 $
...
if (is_qp)
   gf=H*X+f;
   %  SD=-Z*((Z'*H*Z)\(Z'*gf));
   [SD, dirType] = compdirold(Z,H,gf,nvars,f);
...
oldind = 0;
iterqp=0; % Initialize iteration counter added by GvW

% The maximum number of iterations for a simplex type method is:
% maxiters = prod(1:ncstr)/(prod(1:nvars)*prod(1:max(1,ncstr-nvars)));
maxiters=max([10*ncstr,100*nvars,1]); % Maximum number of iterations added by GvW

%--------------Main Routine-------------------
% while 1
while iterqp <= maxiters
 if iterqp==maxiters;
   disp('  Maximum number of iterations exceeded in qpsubold')
   X=[];lambda=[];how='ok';
 else
      ...
      if strcmp(dirType, NewtonStep)
         ...
         if (~is_qp & ~LLS) | (smallRealEig < -eps) % LP or neg def: not LLS
            ...
            if norm(projH*projSD+Zgf) > 10*eps*(norm(projH) + norm(Zgf))
               % system is incompatible --> it's a "chute": use SD from compdirold
 ...
 iterqp=iterqp+1; % Iteration counter added by GvW
end % while 1
