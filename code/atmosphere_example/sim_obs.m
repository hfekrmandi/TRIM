...
for idx_agent=1:opt_dist.nAgents
    ...
    if opt_dist.FLAGS.our_method
        opt_dist.sim.obs.z{idx_agent} = z;
        opt_dist.sim.obs.r_var{idx_agent} = r;
        opt_dist.sim.obs.id_vis{idx_agent} = id_vis;
    end
    ...
    if opt_dist.FLAGS.compare_with_CI
        opt_dist.sim.obs.z_CI{idx_agent} = z;
        opt_dist.sim.obs.r_CI_var{idx_agent} = r;
        opt_dist.sim.obs.id_vis_CI{idx_agent} = id_vis;
        %
    end
end
