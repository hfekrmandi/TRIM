...
function [currentFigure,figureHandle] = GetFigure_estimates_rel(SIM,objectIndex,DATA,currentFigure)

% CONFIGURE THE PLOT ATTRIBUTES
% figurePath = strcat(SIM.outputPath,'isometric');
% figureHandle = figure('Name','OpenMAS estimated trajectories view');
...
for ID1 = 1:DATA.totalAgents 
    ...
    % setappdata(figureHandle, 'SubplotDefaultAxesLocation', [0.08, 0.08, 0.90, 0.88]); % MAXIMISE GRAPH SIZE IN WINDOW
    ax = axes(figureHandle);
    legendCounter = 1; legendEntries = cell(DATA.totalAgents,1);
...
%     end  
    ...
    for ID2 = 1:DATA.totalAgents
        if SIM.OBJECTS(ID2).type == 1 && ID1 ~= ID2
            legendString = sprintf('[Agent-%s] estimating [Agent-%s]',num2str(SIM.OBJECTS(ID2).objectID),num2str(SIM.OBJECTS(ID1).objectID));
            legendEntries(legendCounter) = {legendString};
    ...
    % Title
    title(ax,...
        sprintf('Estimated trajectories of agent %d over a period of %ss', ID1, num2str(SIM.TIME.endTime)),...
